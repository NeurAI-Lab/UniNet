...
from utilities import metric_utils, train_utils, dist_utils
from utilities.infer_utils import run_inference
...
class Trainer:
    ...
    def __init__(self, args):
        ...
        self.norm_layer = train_utils.get_norm_layer(cfg, args)
        ...
        self.model = train_utils.get_model(cfg, args, self.norm_layer, self.tasks)
    ...
    def train_and_evaluate(self):
        saver = train_utils.ModelSaver(self.root_dir, self.task_to_min_or_max)
        start_time = time.time()
        for epoch in range(self.start_epoch, self.args.epochs + 1):
            self.train_one_epoch(epoch)

            task_to_metrics = {task: None for task in self.tasks}
            if epoch == 1 or epoch % 2 == 0 or epoch == self.args.epochs:
    ...
    def train_one_epoch(self, epoch):
        ...
        for cnt, (images, targets) in enumerate(
            ...
            loss, loss_dict = self.mtl_loss(predictions, targets, epoch=epoch - 1)
    ...
    def evaluate(self, epoch=None):
        ...
        with torch.no_grad():
            for cnt, (images, targets, image_idxs) in enumerate(
                ...
                boxes, predictions = run_inference(predictions, cfg, self.tasks)
