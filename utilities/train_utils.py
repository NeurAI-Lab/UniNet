...
from encoding_custom import get_multitask_model
...
def parse_args():
    ...
    parser.add_argument(
        "--eval-only", dest="eval_only", help="Only test the model", action="store_true")
    parser.add_argument(
    ...
    parser.add_argument(
        '--data-folder', type=str, default=os.path.join('/data/input/datasets/Cityscapes'),
        help='training dataset folder (default: $(HOME)/data)')
    ...
    parser.add_argument(
        '--epochs', default=140, type=int, metavar='N', help='number of total epochs to run')
    parser.add_argument(
        '--crop-size', type=int, default=[512, 1024], nargs="*", help='crop image size')
    parser.add_argument(
    ...
    # Optimizer...
    parser.add_argument(
        '--base-optimizer', type=str, default="RAdam", help='optimizer')
    parser.add_argument(
...
def update_config_node(cfg, args, dict_cfg=None):
    ...
    cfg_dict = {name: value for name, value in defaults.__dict__.items()
...
class BatchCollatorFcos(object):
    """
...
def get_dataloader(cfg, args, dataset_train, dataset_val):
    ...
    dl_train = torch.utils.data.DataLoader(
        ...
        collate_fn=BatchCollatorFcos(cfg, dataset=dataset_train),
    ...
    dl_val = torch.utils.data.DataLoader(
        ...
        collate_fn=BatchCollatorFcos(cfg, dataset=dataset_val, val=True),
    ...
    if args.resume:
        ...
        for key in checkpoint['model']:
            if 'pretrained' in key:
                replaced_ckpt['model'][key.replace(
                    'pretrained', 'backbone')] = checkpoint['model'][key]
                del replaced_ckpt['model'][key]
...
class ModelSaver:
    ...
    def __init__(self, save_root, task_to_min_or_max, base_name='model',
                 best_name='best', latest_name='latest',
                 models_to_keep=3):
        self.save_root = save_root
        self.task_to_min_or_max = task_to_min_or_max
        self.base_name = base_name
        self.best_name = best_name
        self.latest_name = latest_name
        self.models_to_keep = models_to_keep
        self.task_to_best = {task: None for task in self.task_to_min_or_max.keys()}
